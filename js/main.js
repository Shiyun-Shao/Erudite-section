// window.onload = function() {
  ...
  function makeGame(url,num) {
      ...
      img.onload = function () {
          ...
          var myw = Math.floor(imgWidth / num),
                  myh = Math.floor(imgHeight / num);
      ...
      function tipNextStep() {
          ...
          if (vvg.step < 5) {
              playBtn.onclick = function () {
                  ...
                  makeGame('images/0' + vvg.step + '.jpg',num+1);
              }
          } else {
              playBtn.onclick = function () {
                  while (canvas.firstChild) {
                  ...
                  desTip.innerHTML = '呦呵~现在你可以自定义图片了，在下面的文本框中输入图片的地址就可以咯，不懂来问我：';
                  var imgDiy = getId('imgDiy');
                  var startDiy = getId('startDiy');
                  var imgUrl = getId('imgUrl');
                  var imgNum = getId('imgNum');
                  imgDiy.style.display = 'block';

                  startDiy.onclick = function () {
                      if (imgUrl.value == '') {
                          alert('笨蛋，请填入图片地址');
                      } else if(imgNum.value == 3){
                          makeGame(imgUrl.value,3);
                          imgUrl.value = '';
                      } else if (imgNum.value == 4) {
                          makeGame(imgUrl.value,4);
                          imgUrl.value = '';
                      } else if (imgNum.value == 5) {
                          makeGame(imgUrl.value,5);
                          imgUrl.value = '';
